...
> - **✅ godlp ENFORCE LIVE + verify (18:36 CEST — SAU mốc 24h 18:28, không rút ngắn):** timer flip `DLP_HOOK_MODE=enforce` → **bằng chứng cuối:** prompt PII "CMND 123456789, SĐT 0912345678, test@gmail.com" → model trả về `**** ******789, *******678, ****@*****.***` (model NHẬN BẢN ĐÃ MASK); log `[DLP-ENFORCE] findings={phone_vn,cmnd_9,email} masked=True`. Rollback: `DLP_HOOK_MODE=off`.
> - **🔧 SỰ CỐ TRONG LÚC BẬT (phát hiện + sửa tận gốc):** restart Coolify **regenerate docker-compose từ DB** → mất file hook `/app/litellm_dlp_hook.py` (trước đó chỉ `docker cp` — không bền) → 8 worker crash-loop (147 lần), LiteLLM sập ~25 phút. Fix: mount hook vào compose file **VÀ ghi vào Coolify DB** (`services.docker_compose_raw`) → bền qua mọi regenerate. LiteLLM UP lại, verify enforce pass.
> - **📚 Docs VI LIVE + verify:** `/vi/docs/introduction` (+core-concepts, why-deerflow, harness-vs-app, tổng quan) → **200, nội dung tiếng Việt thật**; /en giữ 200. 3 vá cần nhớ: thêm `vi` vào `next.config.js i18n.locales`; cần top-level `content/vi/{_meta.ts,index.mdx}`; `_meta` vi CHỈ chứa key có trang thật.
