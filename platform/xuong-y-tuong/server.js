...
async function doiChieuKho(tu_khoa) {
  ...
  for (const [ten, url] of [
    ["van_de", `${BIGDATA}/api/van-de`],
    ["giai_phap", `${BIGDATA}/api/giai-phap`],
    ["mo_hinh", `${BIGDATA}/api/mo-hinh-kd`],
    ["san_pham", `${BIGDATA}/api/san-pham`],
  ]) {
    try {
      const d = await layJson(url, { timeout: 20000 });
      ra[ten] = (d.results || d.items || d[ten] || []).slice(0, 40);
    } catch { ra[ten] = []; }
  }
  try {
...
DỮ LIỆU THẬT ĐỂ ĐỐI CHIẾU (chỉ được dùng những gì có ở đây làm căn cứ):
- Nốt liên quan trong kho: ${JSON.stringify(kho.lien_quan).slice(0, 1500)}
- Vấn đề đã ghi nhận: ${JSON.stringify((kho.van_de || []).slice(0, 8)).slice(0, 1200)}
- Giải pháp đã có: ${JSON.stringify((kho.giai_phap || []).slice(0, 8)).slice(0, 1200)}
- Mô hình KD đã có startup chạy: ${JSON.stringify((kho.mo_hinh || []).slice(0, 8)).slice(0, 1200)}
...
- Trường "can_cu" phải trích id hoặc tên mục thật trong dữ liệu trên.
