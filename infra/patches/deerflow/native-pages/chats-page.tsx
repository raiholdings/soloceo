...
export default function ChatPage() {
  ...
  // SoloCEO OS: khi CEO bấm "Bắt đầu" ở bảng khởi động (Tạo doanh nghiệp),
  // lời nhắc được lưu vào sessionStorage rồi điều hướng sang thread mới này.
  // Đọc 1 lần và tự gửi cho lead_agent để đội AI chạy bước đó (sandbox live + HITL).
  const kickoffFiredRef = useRef(false);
  useEffect(() => {
    if (kickoffFiredRef.current || !isNewThread || isMock) return;
    if (typeof window === "undefined") return;
    let raw: string | null = null;
    try {
      raw = sessionStorage.getItem(KICKOFF_KEY);
    } catch {
      return;
    }
    if (!raw) return;
            ...
            <main className="flex min-h-0 max-w-full grow flex-col">
              ...
              >
                ...
                >
                  ...
                  {isWelcomeMode && mountedRef.current && (
                    <SoloceoProjectTemplates className="mt-4" />
                  )}
                  {env.NEXT_PUBLIC_STATIC_WEBSITE_ONLY === "true" && (
