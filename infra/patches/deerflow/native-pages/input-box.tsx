...
import { SoloceoChipGallery, type ChipKey } from "@/components/workspace/soloceo-chip-gallery";
...
const CHIP_GALLERY: Record<string, ChipKey> = {
  "Khởi tạo ý tưởng": "y-tuong",
  CRM: "crm",
  "Thị trường": "thi-truong",
  "Hệ sinh thái": "he-sinh-thai",
  "Dữ liệu": "du-lieu",
  "Tạo doanh nghiệp": "tao-dn",
};
...
function SuggestionList({
  ...
  const [galleryChip, setGalleryChip] = useState<ChipKey | null>(null);
  ...
  // Chip có gallery: bấm để xem mẫu dữ liệu THẬT bên dưới (không điền prompt ngay)
  const handleChip = useCallback(
    (label: string, prompt: string | undefined) => {
      const key = CHIP_GALLERY[label];
      if (key) { setGalleryChip((cur) => (cur === key ? null : key)); return; }
  ...
  return (
    ...
    <Suggestions className="min-h-16 w-full max-w-full justify-center px-4 sm:w-fit sm:px-0">
      ...
      {t.inputBox.suggestions.map((suggestion) => (
        <Suggestion
          ...
          onClick={() => handleChip(suggestion.suggestion, suggestion.prompt)}
        />
      ))}
      <DropdownMenu>
    ...
    <SoloceoChipGallery chip={galleryChip} onPick={(p) => { handleSuggestionClick(p); setGalleryChip(null); }} />
    </div>
  );
}
