...
def _soloceo_sandbox_url(runtime: str, container_ref: str | None, fallback: str) -> str:
    """SoloCEO OS v2 (N1+N7): sandbox chạy trên network INTERNAL (Internal=true).
    Docker BỎ QUA published port trên network internal ⇒ không thể điều khiển
    sandbox qua host:port (host.docker.internal:8080 rơi vào coolify-proxy).
    Gateway đã được nối vào cùng network ⇒ điều khiển THẲNG qua IP container :8080.
    Env SOLOCEO_SANDBOX_NETWORK rỗng ⇒ giữ nguyên hành vi cũ (fallback)."""
    net = os.environ.get("SOLOCEO_SANDBOX_NETWORK")
    if runtime != "docker" or not net or not container_ref:
        return fallback
    try:
    ...
    except Exception as e:  # fail-safe: rơi về hành vi cũ, không chặn create
        logger.warning(f"SoloCEO: không lấy được IP container {container_ref} trên network {net}: {e}")
    return fallback
...
class LocalContainerBackend(SandboxBackend):
    ...
    def create(
        ...
        *,
        ...
        return SandboxInfo(
            ...
            sandbox_url=_soloceo_sandbox_url(self._runtime, container_id, f"http://{sandbox_host}:{port}"),
    ...
    def discover(self, sandbox_id: str) -> SandboxInfo | None:
        ...
        # SoloCEO: trên network internal, published port bị Docker bỏ qua ⇒ port
        # có thể None. URL theo IP container không cần port host — đừng loại sớm.
        sandbox_host = os.environ.get("DEER_FLOW_SANDBOX_HOST", "localhost")
        fallback_url = f"http://{sandbox_host}:{port}" if port is not None else ""
        sandbox_url = _soloceo_sandbox_url(self._runtime, container_name, fallback_url)
        if not sandbox_url:
            return None
    ...
    def list_running(self) -> list[SandboxInfo]:
        ...
        for container_name in container_names:
            ...
            sandbox_url = _soloceo_sandbox_url(
                self._runtime,
                container_name,
                f"http://{sandbox_host}:{host_port}" if host_port else "",
            )
    ...
    def _start_container(
        ...
        # SoloCEO OS v2 (N1+N7): ép sandbox vào network internal — egress CHỈ qua g3,
        # không internet trực tiếp. Env SOLOCEO_SANDBOX_NETWORK rỗng => hành vi cũ.
        import os as _os
        _net = _os.environ.get("SOLOCEO_SANDBOX_NETWORK")
        if self._runtime == "docker" and _net:
            cmd.extend(["--network", _net])
